...
(function ($) {
	...
	var init = function(plot) {
		...
		var timestampToDate = function(tmst, dateFormat) {
			if (tmst>10000000)
				tmst= tmst-(3600*1000)
		...
		var stringFormat = function(content, item, fnct) {
			...
			if( typeof(fnct) === 'function' ) {
				content = content.replace(xPattern, fnct(item.series.data[item.dataIndex][0], opts.tooltipOpts.dateFormat) );
			}
			...
			// yVal match
			if (opts.yaxis.mode === "time" || opts.yaxes[0].mode === "time") {
				content = content.replace(yPattern, timestampToDate(item.series.data[item.dataIndex][1], opts.tooltipOpts.dateFormat_y));
			} else 	if( typeof item.series.data[item.dataIndex][1] === 'number' ) {
